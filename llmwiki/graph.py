...
from llmwiki.build import FONT_AWESOME_CSS
...
<body>
<a href="#main-content" class="skip-link" data-i18n="graph.skip">跳到正文</a>
__SITE_NAV__
<main id="main-content">
<div id="header">
  <h1 data-i18n="graph.title">llmwiki — 知识图谱</h1>
  <span class="crumbs" id="top-crumbs"></span>
  <span class="spacer"></span>
  <label class="control" title="按节点标签筛选" data-i18n-title="graph.searchTitle">
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
    <input id="search-input" type="search" placeholder="搜索节点…（输入即可筛选）" data-i18n-placeholder="graph.searchPlaceholder" autocomplete="off">
  </label>
  <button class="control" id="cluster-toggle" title="按课程或类型分组节点" data-i18n-title="graph.clusterTitle">
    <span data-i18n="graph.clusterLabel">分组：</span><b id="cluster-mode" data-i18n="graph.clusterOff">关闭</b>
  </button>
...
<div id="network">
  <div id="offline-notice" data-i18n="graph.offlineNotice">`vis-network`（图谱渲染库）加载失败，请检查网络连接或改为本地托管。</div>
  <div id="legend" aria-label="节点颜色图例" data-i18n-aria="graph.legendAria">
    <div class="row"><span class="dot" style="background: var(--g-node-source)"></span><span data-i18n="graph.legendSource">源码页</span></div>
    <div class="row"><span class="dot" style="background: var(--g-node-entities)"></span><span data-i18n="graph.legendEntities">实体页</span></div>
    <div class="row"><span class="dot" style="background: var(--g-node-concepts)"></span><span data-i18n="graph.legendConcepts">概念页</span></div>
    <div class="row"><span class="dot" style="background: var(--g-node-syntheses)"></span><span data-i18n="graph.legendSyntheses">综述页</span></div>
    <div class="row"><span class="dot" style="border: 2px solid var(--g-orphan); background: transparent"></span><span data-i18n="graph.legendOrphans">孤立页</span></div>
  </div>
  <div id="stats-overlay" aria-label="图谱统计" data-i18n-aria="graph.statsAria">
    <h3 data-i18n="graph.statsTitle">统计</h3>
    <div class="stat"><span data-i18n="graph.pages">页面</span><b id="s-pages">0</b></div>
    <div class="stat"><span data-i18n="graph.edges">连边</span><b id="s-edges">0</b></div>
    <div class="stat"><span data-i18n="graph.orphans">孤立页</span><b id="s-orphans">0</b></div>
    <div class="stat"><span data-i18n="graph.avgDegree">平均连接数</span><b id="s-avg">0</b></div>
    <h3 style="margin-top: 10px;" data-i18n="graph.coreNodes">核心节点</h3>
    <div id="s-hubs"></div>
  </div>
  <!-- G-19 (#305) node context menu — right-click / long-tap target -->
  <div id="ctx-menu" role="menu" aria-label="节点操作" data-i18n-aria="graph.ctxAria">
    <div class="ctx-header" id="ctx-target">—</div>
    <button type="button" role="menuitem" data-action="open"><span data-i18n="graph.openPage">打开页面</span> <span class="ctx-kbd">Enter</span></button>
    <button type="button" role="menuitem" data-action="neighbours"><span data-i18n="graph.neighbours">查看相邻节点（1 跳）</span> <span class="ctx-kbd">N</span></button>
    <button type="button" role="menuitem" data-action="copy-slug"><span data-i18n="graph.copySlug">复制 `slug`（页面标识）</span> <span class="ctx-kbd">C</span></button>
    <button type="button" role="menuitem" data-action="copy-path" data-i18n="graph.copyPath">复制 `wiki`（知识库）路径</button>
    <button type="button" role="menuitem" data-action="view-references" data-i18n="graph.copyReferences">复制引用查询命令</button>
    <div class="ctx-separator" role="separator"></div>
    <button type="button" role="menuitem" data-action="mark-stale" disabled
            title="需要 `llmwiki serve --edit`（暂未提供）" data-i18n-title="graph.editRequired">
      <span data-i18n="graph.markStale">标记为过期</span>
    </button>
    <button type="button" role="menuitem" data-action="archive" disabled
            title="需要 `llmwiki serve --edit`（暂未提供）" data-i18n-title="graph.editRequired">
      <span data-i18n="graph.archive">归档</span>
    </button>
  </div>
</div>
</main>
...
const GRAPH_I18N = {
  "zh-Hans": {
    ...
    skip: "跳到正文",
    searchTitle: "按节点标签筛选",
    searchPlaceholder: "搜索节点…（输入即可筛选）",
    clusterTitle: "按课程或类型分组节点",
    clusterLabel: "分组：",
    clusterOff: "关闭",
    clusterType: "按类型",
    offlineNotice: "`vis-network`（图谱渲染库）加载失败，请检查网络连接或改为本地托管。",
    legendAria: "节点颜色图例",
    legendSource: "源码页",
    legendEntities: "实体页",
    legendConcepts: "概念页",
    ...
    ctxAria: "节点操作",
    openPage: "打开页面",
    neighbours: "查看相邻节点（1 跳）",
    copySlug: "复制 `slug`（页面标识）",
    copyPath: "复制 `wiki`（知识库）路径",
    copyReferences: "复制引用查询命令",
    markStale: "标记为过期",
    archive: "归档",
    editRequired: "需要 `llmwiki serve --edit`（暂未提供）",
    topCrumbs: function (pages, edges, orphans) {
      return pages + " 个页面 · " + edges + " 条连边 · " + orphans + " 个孤立页";
    },
    nodeTitle: function (type, inDegree, outDegree, path) {
      return type + " · 入边 " + inDegree + "，出边 " + outDegree + (path ? "\n点击打开 " + path : "");
    },
    noSiteTip: function (label) {
      return label + " —— 暂无已生成页面（可查看“## Connections”）";
    },
    noSiteAlert: function (label) {
      return label + " —— 这个节点还没有已生成页面。实体页、概念页和导航页仍保留在 `wiki/`（知识库目录）里，不会单独渲染成站点页面。";
    },
    referencesCopied: function (slug) {
  ...
  "zh-Hant": {
    ...
    skip: "跳到正文",
    searchTitle: "按節點標籤篩選",
    searchPlaceholder: "搜尋節點…（輸入即可篩選）",
    clusterTitle: "按課程或類型分組節點",
    clusterLabel: "分組：",
    clusterOff: "關閉",
    clusterType: "按類型",
    offlineNotice: "`vis-network`（圖譜渲染庫）載入失敗，請檢查網路連線或改為本地託管。",
    legendAria: "節點顏色圖例",
    legendSource: "原始頁面",
    legendEntities: "實體頁",
    legendConcepts: "概念頁",
    ...
    ctxAria: "節點操作",
    openPage: "打開頁面",
    neighbours: "查看相鄰節點（1 跳）",
    copySlug: "複製 `slug`（頁面標識）",
    copyPath: "複製 `wiki`（知識庫）路徑",
    copyReferences: "複製引用查詢命令",
    markStale: "標記為過期",
    archive: "封存",
    editRequired: "需要 `llmwiki serve --edit`（暫未提供）",
    topCrumbs: function (pages, edges, orphans) {
      return pages + " 個頁面 · " + edges + " 條連邊 · " + orphans + " 個孤立頁";
    },
    nodeTitle: function (type, inDegree, outDegree, path) {
      return type + " · 入邊 " + inDegree + "，出邊 " + outDegree + (path ? "\n點擊打開 " + path : "");
    },
    noSiteTip: function (label) {
      return label + " —— 暫無已生成頁面（可查看「## Connections」）";
    },
    noSiteAlert: function (label) {
      return label + " —— 這個節點還沒有已生成頁面。實體頁、概念頁和導覽頁仍保留在 `wiki/`（知識庫目錄）裡，不會單獨渲染成站點頁面。";
    },
    referencesCopied: function (slug) {
      return "已複製命令到剪貼簿：\n\n  llmwiki references \"" + slug + "\"";
    }
  ...
  en: {
    title: "llmwiki — Knowledge Graph",
    skip: "Skip to main content",
    searchTitle: "Filter by node label",
    searchPlaceholder: "Search nodes… type to filter",
    clusterTitle: "Group nodes by course or type",
    clusterLabel: "Group:",
    clusterOff: "Off",
    clusterType: "By type",
    offlineNotice: "`vis-network` failed to load. Check your network connection or host it locally.",
    legendAria: "Node color legend",
    legendSource: "Source pages",
    legendEntities: "Entity pages",
    legendConcepts: "Concept pages",
    legendSyntheses: "Synthesis pages",
    legendOrphans: "Orphan pages",
    statsAria: "Graph stats",
    statsTitle: "Stats",
    pages: "Pages",
    edges: "Edges",
    orphans: "Orphans",
    ...
    ctxAria: "Node actions",
    openPage: "Open page",
    neighbours: "Show neighbours (1 hop)",
    copySlug: "Copy slug",
    copyPath: "Copy wiki path",
    copyReferences: "Copy references command",
    markStale: "Mark stale",
    archive: "Archive",
    editRequired: "Requires `llmwiki serve --edit` (not available yet)",
    topCrumbs: function (pages, edges, orphans) {
      return pages + " pages · " + edges + " edges · " + orphans + " orphan pages";
    },
    nodeTitle: function (type, inDegree, outDegree, path) {
      return type + " · in " + inDegree + ", out " + outDegree + (path ? "\nClick to open " + path : "");
    },
    noSiteTip: function (label) {
      return label + " — no generated page yet (see ## Connections)";
    },
    noSiteAlert: function (label) {
      return label + " — this node does not have a generated page yet. Entity, concept, and navigation pages stay in `wiki/` and are not rendered as standalone site pages.";
    },
    referencesCopied: function (slug) {
      return "Copied command to clipboard:\n\n  llmwiki references \"" + slug + "\"";
    }
  }
};

function getGraphLang() {
  try {
    const raw = localStorage.getItem('look-tongji-lang') || document.documentElement.lang || 'zh-Hans';
    if (raw === 'zh' || raw === 'zh-CN' || raw === 'zh-SG' || raw === 'zh-Hans') return 'zh-Hans';
    if (raw === 'zh-TW' || raw === 'zh-HK' || raw === 'zh-MO' || raw === 'zh-Hant') return 'zh-Hant';
    if (raw === 'en' || /^en-/.test(raw)) return 'en';
    return GRAPH_I18N[raw] ? raw : 'zh-Hans';
  } catch (_) {
    return 'zh-Hans';
  }
}
...
function graphText() {
  return GRAPH_I18N[getGraphLang()] || GRAPH_I18N.en;
}

function applyGraphI18n() {
  const text = graphText();
  document.querySelectorAll('[data-i18n]').forEach(function (node) {
    const key = node.getAttribute('data-i18n').replace(/^graph\./, '');
    if (text[key]) node.textContent = text[key];
  });
  document.querySelectorAll('[data-i18n-title]').forEach(function (node) {
    const key = node.getAttribute('data-i18n-title').replace(/^graph\./, '');
    if (text[key]) node.setAttribute('title', text[key]);
  });
  document.querySelectorAll('[data-i18n-placeholder]').forEach(function (node) {
    const key = node.getAttribute('data-i18n-placeholder').replace(/^graph\./, '');
    if (text[key]) node.setAttribute('placeholder', text[key]);
  });
  document.querySelectorAll('[data-i18n-aria]').forEach(function (node) {
    const key = node.getAttribute('data-i18n-aria').replace(/^graph\./, '');
    if (text[key]) node.setAttribute('aria-label', text[key]);
  });
  document.title = text.title;
}

const GRAPH_CHROME_IDS = ['offline-notice', 'legend', 'stats-overlay', 'ctx-menu'];
const GRAPH_CHROME_TEMPLATES = {};
GRAPH_CHROME_IDS.forEach(function (id) {
  const node = document.getElementById(id);
  GRAPH_CHROME_TEMPLATES[id] = node ? node.outerHTML : '';
});
...
function ensureGraphChrome() {
  ...
  if (!container) return;
  GRAPH_CHROME_IDS.forEach(function (id) {
    if (document.getElementById(id) || !GRAPH_CHROME_TEMPLATES[id]) return;
    container.insertAdjacentHTML('beforeend', GRAPH_CHROME_TEMPLATES[id]);
  });
}
...
function main() {
  ...
  document.getElementById('top-crumbs').textContent = text.topCrumbs(pages, edgeCount, orphans.length);
  ...
  const nodes = new vis.DataSet(GRAPH.nodes.map(n => {
    ...
    return {
      ...
      group: n.type,
      title: text.nodeTitle(n.type, n.in_degree, n.out_degree, n.path),
      path: n.path,
      type: n.type,
      site_url: n.site_url,
    };
  ...
  window.__llmwikiGraphNetwork = network;
  ...
  function _flashNoSiteTooltip(node, ev) {
    ...
    tip.textContent = graphText().noSiteTip(node.label);
    tip.style.cssText =
    ...
    switch (action) {
      case 'open': {
        ...
        if (node.site_url) {
          ...
          alert(graphText().noSiteAlert(node.label));
        }
        break;
      }
      case 'neighbours':
      ...
      case 'view-references': {
        ...
        alert(graphText().referencesCopied(slug));
        break;
      }
      default:
  ...
  if (clusterBtn) clusterBtn.addEventListener('click', () => {
    ...
    document.getElementById('cluster-mode').textContent = clusterMode === 'type' ? graphText().clusterType : graphText().clusterOff;
    if (clusterMode === 'type') {
...
window.addEventListener('look-tongji:langchange', function () {
  applyGraphI18n();
  if (!window.__llmwikiGraphNetwork) return;
  try {
    window.__llmwikiGraphNetwork.destroy();
  } catch (_) {}
  window.__llmwikiGraphNetwork = null;
  const container = document.getElementById('network');
...
def write_html(graph: dict[str, Any], out_path: Path) -> None:
    ...
    html = (
        ...
        .replace("__FONT_AWESOME_CSS__", FONT_AWESOME_CSS)
    )
