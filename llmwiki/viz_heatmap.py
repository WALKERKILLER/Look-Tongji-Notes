...
def render_heatmap(
    ...
    parts: list[str] = [
        ...
        f'data-heatmap-start="{start.isoformat()}" data-heatmap-end="{end.isoformat()}" '
        f'data-heatmap-title-prefix="{html.escape(title_prefix)}" '
        f'data-heatmap-kind="{"project" if title_prefix.endswith("活跃度") and title_prefix != "活跃度" else "aggregate"}" '
        f'data-heatmap-project-name="{html.escape(title_prefix[:-3]) if title_prefix.endswith("活跃度") and title_prefix != "活跃度" else ""}">',
    ...
    for col in range(WEEK_COLS):
        ...
        if col_date.day <= 7:
            ...
            parts.append(
                f'<text x="{x}" y="{TOP_PAD - 6}" data-month="{col_date.month}">'
    ...
    for row, label_text in enumerate(_WEEKDAY_LABELS):
        ...
        parts.append(
            f'<text x="0" y="{y}" data-weekday="{row}">{label_text}</text>'
        )
